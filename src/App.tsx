...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'PLANNER' | 'WEATHER' | 'LOGS' | 'HANGAR' | 'TOOLS'>('PLANNER');
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-black text-zinc-100 font-mono overflow-hidden border border-zinc-800 shadow-2xl">
      ...
      <nav 
        ...
        className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900 px-4 py-2 select-none shrink-0"
      >
        ...
        <div className="flex items-center gap-1 overflow-x-auto no-scrollbar bg-black border border-zinc-800 rounded-sm px-1" style={{ ["--wails-draggable" as any]: "no-drag" }}>
          {(['PLANNER', 'WEATHER', 'LOGS', 'HANGAR', 'TOOLS'] as const).map((tab) => (
            ...
            >
              {/* Responsive Labeling */}
              <span className="sm:hidden">{tab.substring(0, 4)}</span>
      ...
      <main className="flex-1 overflow-y-auto p-4 lg:p-8 max-w-7xl mx-auto w-full custom-scrollbar" style={{ WebkitOverflowScrolling: 'touch' } as any}>
      ...
      {/* 📟 STATUS BAR */}
      <footer className="h-8 bg-zinc-900 border-t border-zinc-800 px-4 flex items-center justify-between text-[9px] text-zinc-600 uppercase tracking-widest shrink-0">
        <div className="flex gap-4">
          <span className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-green-500"></div> Nominal</span>
          <span>Logs: {flights.length}</span>
        </div>
        <div className="text-red-900 font-bold hidden sm:block">100% OFFLINE // PRE-FLIGHT_SUITE</div>
      </footer>
    </div>
  );
};
